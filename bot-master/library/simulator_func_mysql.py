...
class simulator_func_mysql:
    ...
    def trade_check(self, df_row, open_price, current_price, current_sum_volume):
        ...
        # 래리 윌리엄스 변동성 돌파 알고리즘 + 거래대금(매수)
            ...
            current_total_tr_price = current_price * current_sum_volume
            _range = yes_high - yes_low
            if open_price + _range * self.rarry_k < current_price and current_price > yes_close and current_total_tr_price > yes_total_tr_price * self.volume_up:
                return True
            else:
                return False
    ...
    def db_to_realtime_daily_buy_list(self, date_rows_today, date_rows_yesterday, i):
        ...
        # 5 / 40 골든크로스 buy
            ...
            sql = "select * from `" + date_rows_yesterday + "` a " \
                                                            "where volume > 3000000 " \
                                                            "and close < '%s' and NOT exists (select null from stock_konex b where a.code=b.code) " \
                                                            "group by code order by volume desc limit 20 "
            realtime_daily_buy_list = self.engine_daily_buy_list.execute(sql % (self.invest_unit)).fetchall()
    ...
    def get_sell_list(self, i):
        ...
        # 5 / 20 이동 평균선 데드크로스 이거나, losscut_point(손절 기준 수익률) 이하나 sell_point(익절 기준 수익률) 이되면 판매하는 알고리즘
        elif self.sell_list_num == 2:
            sql = "SELECT code, rate, present_price,valuation_profit FROM all_item_db WHERE (sell_date = '%s') " \
                  "and ((clo5 < clo20) or rate <= '%s' or rate > '%s') group by code"
            sell_list = self.engine_simulator.execute(sql % (0, self.losscut_point, self.sell_point)).fetchall()
        ...
        # 익일 매도
            ...
            # min_date_rows_join = ''.join(self.min_date_rows[0])
            sell_data = ''.join(self.date_rows[i-1])
            sql = "SELECT code, rate, present_price,valuation_profit FROM all_item_db " \
                  "WHERE sell_date = '%s' and (buy_date like '%s' or rate>='%s' or rate <= '%s')  group by code "
            sell_list = self.engine_simulator.execute(sql % (0, sell_data, self.sell_point, self.losscut_point)).fetchall()
